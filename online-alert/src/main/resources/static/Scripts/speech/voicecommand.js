...
(function (a) {
    ...
    var e = {
        start: [],
        error: [], end: [], result: [], resultMatch: [], resultNoMatch: [], errorNetwork: [], errorPermissionBlocked: [], errorPermissionDenied: []
    };
    var f = 0,
        g = !1,
        ...
        m = /[\-{}\[\]+?.,\\\^$|#]/g;
    var n = function (a) {
        return a = a.replace(m, "\\$&").replace(i, "(?:$1)?").replace(k, function (a, b) { return b ? a : "([^\\s]+)" }).replace(l, "(.*?)").replace(j, "\\s*$1?\\s*"), new RegExp("^" + a + "$", "i")
    };
    var o = function (a) {
        a.forEach(function (a) { a.callback.apply(a.context) })
    };
    var p = function () {
        ...
        addCommands: function (a) {
            ...
            for (var e in a)
                if (a.hasOwnProperty(e)) {
                    if (b = root[a[e]] || a[e], "function" != typeof b) continue; c = n(e), d.push({ command: c, callback: b, originalPhrase: e })
                }
            g && root.console.log("Commands successfully loaded: %c" + d.length, h)
        },
        removeCommands: function (b) {
            return b === a ? void (d = []) : (b = Array.isArray(b) ? b : [b], void (d = d.filter(function (a) { for (var c = 0; c < b.length; c++) if (b[c] === a.originalPhrase) return !1; return !0 })))
        },
        addCallback: function (b, c, d) {
            if (e[b] !== a) { var f = root[c] || c; "function" == typeof f && e[b].push({ callback: f, context: d || this }) }
        }
    }
}).call(this);
var autoStart = function () {
    smartSpeechRecognition.addCommands(commands),
    smartSpeechRecognition ? (smartSpeechRecognition.start(), $.root_.addClass("voice-command-active"), voice_localStorage && localStorage.setItem("sm-setautovoice", "true")) : alert("主意插件未加载")
};
