	...
	var bigAutocomplete = new function(){
		...
		this.autocomplete = function(param){
			...
			this.config = {
			               ...
			               url和data、getCityFunction参数只有一个生效，data优先, url 次之, getCityFunction最低*/
			               data: null,
                           getCityFunction: null,
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					var url = config.url;
					var getCityFunction = config.getCityFunction;
					...
					} else if ($.isFunction(getCityFunction)) {
					    if ($.trim(getCityFunction()) != "") {
					        var options = {
					            onSearchComplete: function (results) {

					                if (localSearch.getStatus() == BMAP_STATUS_SUCCESS) {
					                    var s = convertMapSearch(results);
					                    makeContAndShow(s);
					                }
					            }
					        };

					        var localSearch = new BMap.LocalSearch(getCityFunction(), options);
					        localSearch.search(keyword_);
					    }
					}
			...
			function convertMapSearch(resultList) {
			    var arr = [];
			    for (var i = 0; i < resultList.getCurrentNumPois() ; ++i) {
			        if (i >= 5) {
			            break;
			        }
			        var itm = resultList.getPoi(i);
			        arr.push({ title: itm.title, result: itm });
			    }
			    return arr;
			}
