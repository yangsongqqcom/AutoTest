...
ko.bindingHandlers.editor = (function () {

    function addValidate(element, options) {
        if (options.validate) {
            if (options.validate.type && options.validate.type == "jq") {
                var funContent = "return {" + $(element).attr("data-options") + "};";
                var jsonConfig = new Function(funContent)();
                $("#" + options.editorKey).validatebox(jsonConfig);
            }
        }
    }

    function setEditorValue(valueUnwrapped, element, options) {
        if (options.validate) {
            if (options.validate.type && options.validate.type == "jq") {
                $("#" + options.editorKey).val(valueUnwrapped);
            }
        }

        if (options.editor.getContent() != valueUnwrapped)
            options.editor.setContent(valueUnwrapped, false);
    }

    return {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var value = valueAccessor(), allAccessor = allBindingsAccessor();
            var options = allAccessor.binding || {};
            ...
            options.editor.addListener('ready', function (editor) {
                options.editor.isLoad = true;
                addValidate(element, options);
            });

            var v = value();
            if (v != undefined) {
                if (!(options.editor.isLoad && options.editor.isLoad == true)) {
                    try {
                        options.editor.setContent(valueUnwrapped, false);
                    } catch (e) {
                        options.editor.addListener('ready', function (editor) {
                            options.editor.isLoad = true;
                            addValidate(element, options);
                            setEditorValue(valueUnwrapped, element, options);
                        });
                    }
                } else {
                    setEditorValue(valueUnwrapped, element, options);
            ...
            options.editor.addListener('blur', function () {
                var inputValue = options.editor.getContent();
                value(inputValue);
            });
        },
